...
const tierPoints = n => Math.max(10, 110 - 10 * n);      // للمخمّن: تلميحة 1=100 .. 10=10
const cluerPoints = n => Math.round(tierPoints(n) / 2);  // للملمّح: النص (50, 45, 40 ...)
/* حدود التلميح حسب عدد الكلمات المسموح (عشان محدش يلزق كلمتين في بعض) */
const MAX_WORD_CHARS = 14;
function hintLimits(maxWords) { return { words: maxWords, perWord: MAX_WORD_CHARS, total: maxWords * MAX_WORD_CHARS + (maxWords - 1) }; }
function checkHintShape(text, maxWords) {
  const words = String(text).trim().split(/\s+/).filter(Boolean);
  if (!words.length) return 'اكتب تلميح';
  if (words.length > maxWords) return `التلميح لازم يكون ${maxWords} ${maxWords === 1 ? 'كلمة' : 'كلمات'} على الأكتر — انت كتبت ${words.length}`;
  const long = words.find(w => w.length > MAX_WORD_CHARS);
  if (long) return `في كلمة طويلة أوي («${long.slice(0, 16)}») — أقصى ${MAX_WORD_CHARS} حرف للكلمة`;
  return null;
}
...
function pickItem(room) {
  ...
  if (!pool.length) for (const c of room.settings.cats) for (const it of BANK.catItems(c)) if (!room.usedItems.has(it.id)) pool.push(it);
...
function viewFor(room, p) {
  ...
  if (room.phase === 'clue') {
    ...
    st.maxWords = room.settings.maxWords;
    st.hintLimits = hintLimits(room.settings.maxWords);
    if (room.sub === 'pick') {
      st.passesLeft = Math.max(0, room.settings.maxPass - room.passesUsed);
      if (st.youAreCluer) { st.secret = room.item ? room.item.title : null; st.catOptions = room.settings.cats.map(c => BANK.catMeta(c)); }
    }
    ...
    st.passesLeft = Math.max(0, room.settings.maxPass - room.passesUsed);
    const nHint = room.sub === 'guess' ? room.cluesGiven : room.cluesGiven + 1;
    st.tier = tierPoints(nHint);
    st.cluerTier = cluerPoints(nHint);
...
function buildPlan(room) {
  // كل لاعب بيلمّح roundsPerPlayer مرة بالظبط — العشوائي بيغيّر الترتيب بس مش الأعداد
  const base = room.order.slice();
  const seq = [];
  for (let r = 0; r < room.settings.roundsPerPlayer; r++) {
    seq.push(...(room.settings.order === 'random' ? shuffle(base) : base));
  }
  room.plan = seq.map(tok => ({ cluer: tok }));
}
...
function startRound(room) {
  ...
  room.cluer = cluerTok;
  room.customWord = null;
  if (room.settings.allowCustomWord) {
    // الملمّح هيختار: ياخد من البنك أو يكتب كلمته — لسه محددش
    room.item = null;
    room.pickMode = null;
    room.sub = 'pick';
  } else {
...
function endRound(room, winnersTokens, finalEntries) {
  ...
  if (solved) {
    ...
    if (cluer) { cluer.score += cpts; cluer.stat.cluedSuccess++; }
  }
...
module.exports = {
  ...
  action(b) {
    ...
    if (A === 'pickBank') {
      if (room.phase !== 'clue' || room.sub !== 'pick') return R(400, { ok: false, error: 'مش وقتها' });
    ...
    if (A === 'pickCustom') {
      ...
      room.item = { id: 'custom_' + rid().slice(0, 8), cat, level: 'custom', title: word, accepts: [BANK.normalize(word)].filter(Boolean) };
      room.customWord = word;
    ...
    if (A === 'submitHint') {
      ...
      const shape = checkHintShape(text, room.settings.maxWords);
      if (shape) return R(400, { ok: false, error: shape });
    ...
    if (A === 'giveUp') {
      ...
      if (room.sub === 'pick') return R(400, { ok: false, error: 'اختار الكلمة الأول' });
      endRound(room, [], null);
      return R(200, { ok: true });
    }
    if (A === 'pass') {
      if (room.phase !== 'clue' || (room.sub !== 'hint' && room.sub !== 'pick')) return R(400, { ok: false, error: 'مش وقتها' });
      if (room.cluer !== p.token) return R(403, { ok: false, error: 'الملمّح بس' });
      if (room.pickMode === 'custom') return R(400, { ok: false, error: 'انت كاتب الكلمة بنفسك' });
      if (room.cluesGiven > 0) return R(400, { ok: false, error: 'مينفعش تبدّل بعد ما لمّحت' });
