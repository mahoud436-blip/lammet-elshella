...
/* ===== مهلة سماحية لو حد قطع النت (بدل ما القضية تتلغي على طول) ===== */
const ACCUSED_GRACE_MS = HOST_GRACE_MS;   // مهلة رجوع المتّهم
const ASKER_GRACE_MS = 25000;             // مهلة رجوع المحقق اللي عليه الدور
function clearPause(room) { if (room.pauseTimer) { clearTimeout(room.pauseTimer); room.pauseTimer = null; } room.paused = null; }
function pauseCase(room, reason, kind, graceMs) {
  if (room.paused) return;
  clearTimer(room);
  room.paused = { reason, kind, until: now() + graceMs };
  room.pauseTimer = setTimeout(() => { room.pauseTimer = null; onGraceExpire(room); }, graceMs + 200);
  broadcast(room);
}
function neededBack(room) {
  if (!room.paused) return true;
  if (room.paused.kind === 'accused') { const a = room.players.get(room.accused); return !!(a && a.connected); }
  if (room.paused.kind === 'asker') { const tok = room.askOrder[room.askIdx]; const c = tok ? room.players.get(tok) : null; return !!(c && c.connected); }
  return true;
}
function resumeCase(room) {
  if (!room.paused) return;
  clearPause(room);
  if (room.phase === 'play') {
    if (room.sub === 'ask') armAsk(room);
    else if (room.sub === 'answer') armAnswer(room);
  }
  broadcast(room);
}
function onGraceExpire(room) {
  const kind = room.paused ? room.paused.kind : null;
  room.paused = null;
  if (kind === 'asker' && room.phase === 'play' && room.sub === 'ask') { skipAsk(room); return; }
  if (room.phase === 'pick' || room.phase === 'play') { finishCase(room); return; }   // المتّهم مرجعش
  broadcast(room);
}
...
function finishGame(room) {
  ...
  room.results = { ranking, awards, review: room.caseResults.slice() };
  room.phase = 'gameover';
  broadcast(room);
}
...
function playAgain(room) {
  ...
  clearTimer(room); clearPause(room);
  room.plan = []; room.caseIdx = 0; room.caseResults = [];
  room.round = 0; room.history = []; room.submissions = new Map(); room.decided = new Set();
...
function recheckGates(room) {
  if (room.paused) return;   // متوقفة بالفعل — مستنيين اللاعب يرجع
  if (room.phase === 'pick') {
    const acc = room.players.get(room.accused);
    if (!acc || !acc.connected) pauseCase(room, '⏸️ مستنيين المتّهم يرجع...', 'accused', ACCUSED_GRACE_MS);
    return;
  }
  if (room.phase === 'play') {
    const acc = room.players.get(room.accused);
    if (!acc || !acc.connected) { pauseCase(room, '⏸️ مستنيين المتّهم يرجع...', 'accused', ACCUSED_GRACE_MS); return; }
    // لو كل المحققين خرجوا خالص
    if (!connectedPlayers(room).filter(x => x.token !== room.accused).length) { finishCase(room); return; }
    if (room.sub === 'ask') {
      const curTok = room.askOrder[room.askIdx];
      const cur = curTok ? room.players.get(curTok) : null;
      if (!cur || !cur.connected) pauseCase(room, '⏸️ مستنيين المحقق اللي عليه الدور يرجع...', 'asker', ASKER_GRACE_MS);
    }
    if (room.sub === 'decide') maybeCloseDecide(room);
  }
  if (room.phase === 'caseEnd') maybeAdvance(room);
}
...
module.exports = {
  ...
  stream(req, res, code, token) {
    ...
    if (room.paused && neededBack(room)) resumeCase(room);
    broadcast(room);
    req.on('close', () => {
  ...
  action(b) {
    ...
    if (room.paused && !['leave', 'presence', 'kick', 'forceNext', 'playAgain'].includes(A))
      return R(409, { ok: false, error: 'اللعبة متوقفة مؤقتًا — مستنيين اللاعب يرجع ⏸️' });
    ...
    if (A === 'readyNext') {
      ...
      maybeAdvance(room);
      if (room.phase === 'caseEnd') broadcast(room);
      return R(200, { ok: true });
    }
    if (A === 'forceNext') {
      ...
      else if (room.phase === 'caseEnd') advanceCase(room);
      return R(200, { ok: true });
    }
    if (A === 'presence') {
      const away = !!b.away;
