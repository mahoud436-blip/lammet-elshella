...
/* «حد جاوب صح ومتحسبتش؟» — شبكة أمان لو النظام فاته إجابة مظبوطة */
function creditCard(st) {
  ...
  return `<div class="card credit-card">
    ...
    <div class="credit-sub">انت اللي عارف الكلمة — لو شايف إجابة كانت مظبوطة والنظام مقبلهاش، احسبها له من هنا</div>
...
function sigOf(st) {
  ...
  if (P === 'caseEnd') return 'E|' + st.caseNo + '|' + (st.youReady ? 'r' : '') + '|c' + (st.creditCount || 0);
...
function bindLeave2() { const lb = $('#leave-btn2'); if (lb) lb.onclick = async () => { if (await uiConfirm('تخرج من الروم؟ سكورك هيفضل محسوب', { emoji: DOOR_OUT, okLabel: 'اخرج' })) { await act('leave'); leaveLocal(); } }; }
...
function buildGameover(st) {
  ...
  app.innerHTML = `${header('خلصت اللعبة! 🎉')}
    ...
    <div class="card"><h3 class="mb">📊 الترتيب النهائي</h3>${R.ranking.map(p => `<div class="rank-row ${p.id === me.id ? 'me' : ''}"><span class="pos">#${p.rank}</span><span>${p.avatar}</span><span>${esc(p.name)}${p.left ? ' <span class="muted small">${DOOR_OUT}</span>' : ''}</span><span class="sc">${p.score}</span></div>`).join('')}</div>
...
const CONAN_STEPS = [
  ...
  ['💯', 'تسلّم ولا تكمّل؟', 'آخر كل جولة تختار: <span class="hl">تسلّم تخمينك</span> (يتقفل) ولا تكمّل تحقيق. صح في الجولة 1 = <span class="hl">100</span>، الجولة 2 = <span class="hl">90</span>، وهكذا لحد <span class="hl">10</span> — <b>كل ما تتأخر تقل نقطك</b>. وآخر جولة لازم تسلّم.'],
  ['🔒', 'الكشف بعد كل قضية', 'تخمينك بيفضل مقفول و<span class="warn">محدش يعرف صح ولا غلط</span> لحد ما القضية تخلص. ساعتها بتتكشف الكلمة وكل التخمينات ومين عرفها، وتتحسب النقط ويتحدّث الترتيب.'],
  ['⚠️', 'عقوبة المتّهم', 'المتّهم <span class="hl">مبياخدش نقط خالص</span>. ولو الهوست حاطط وقت للرد ومردّش في الوقت، <span class="warn">بيتخصم منه 10 نقط عن كل سؤال</span> فايته.'],
  ['⚙️', 'الهوست بيحدد', 'الكاتيجوريز، كام مرة كل لاعب يبقى متّهم، <span class="hl">عدد أسئلة كل محقق (2–10)</span>، ترتيب الأدوار، وهل المتّهم يكتب كلمته، ووقت السؤال والرد.'],
  [DOOR_OUT, 'قطع النت وحاجات', 'لو حد قطع، اللعبة <span class="hl">بتقف وتستنّاه</span> مبتتلغيش، وتكمّل أول ما يرجع. الكلمة متتكررش في الروم أبدًا، ولو خرجت سكورك بيفضل محسوب.'],
];
function showHelp() {
