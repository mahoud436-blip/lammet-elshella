...
/* مثال إرشادي ثابت لكل كاتيجوري — بيظهر في خانة كتابة السؤال */
const CAT_EXAMPLE = {
  movies: 'مثال: مين بطل فيلم «الكيت كات»؟',
  anime: 'مثال: إيه اسم فاكهة الشيطان بتاعة لوفي في ون بيس؟',
  hist_islam: 'مثال: مين الصحابي الملقّب بسيف الله المسلول؟',
  hist_ar: 'مثال: مين قائد ثورة 1919 في مصر؟',
  geo_ar: 'مثال: إيه أكبر محافظة في مصر من حيث المساحة؟',
  geo_world: 'مثال: إيه أطول نهر في العالم؟',
  religion: 'مثال: كام عدد أركان الإسلام؟',
  sports: 'مثال: مين هدّاف كأس العالم 2022؟',
  sci: 'مثال: إيه أكبر كوكب في المجموعة الشمسية؟',
  mix: 'مثال: إيه عاصمة اليابان؟',
};
const ALL_CATS = [
...
function sendPresence(away) { sendAway(away); }
...
function renderLobby(st) {
  ...
  app.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="mt center muted small">مستوى أسئلة البنك 🎲</div>
      <div class="row wrap" style="justify-content:center">
        ${(st.levels || [{id:'easy',icon:'🟢',name:'سهل'},{id:'medium',icon:'🟡',name:'متوسط'},{id:'hard',icon:'🔴',name:'صعب'}]).map(l => `<span class="chip click ${st.settings.level === l.id ? 'on' : ''} ${isHost ? '' : 'locked'}" data-level="${l.id}">${l.icon} ${l.name}</span>`).join('')}
      </div>
      <div class="center muted small" style="margin-top:4px;font-size:12px">السحب من البنك هيجيب أسئلة من المستوى ده بس</div>
  ...
  if (isHost) {
    ...
    $$('.chip[data-level]').forEach(el => el.onclick = () => act('setSettings', { settings: { level: el.dataset.level } }));
  ...
  app.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="mt"><label class="muted small">السؤال</label>
        <textarea class="field" id="q-in" maxlength="200" placeholder="${esc(CAT_EXAMPLE[slot.cat] || 'اكتب سؤال معلومات واضح')}">${esc(b.q)}</textarea></div>
  ...
  if (q.sub === 'answering') {
    app.innerHTML = `${header('')}
      <div class="card">
        ...
        <div class="choices">
          ${q.choices.map((c, i) => `
            <button class="choice ${CH_CLASS[i]} ${q.yourChoice === i ? 'picked' : ''}" data-c="${i}">
              <span class="shape">${SHAPES[i]}</span><span>${esc(c)}</span>
            </button>`).join('')}
        </div>
        <div class="answered-strip" id="answered">${answeredAvatars(st)}</div>
        <div class="center muted small">جاوبوا <b id="ans-count">${q.answeredIds.length}</b> من ${q.eligible}</div>
        <div class="center muted small" id="ans-note" style="font-size:12px">${q.yourChoice != null && q.yourChoice >= 0 ? 'إجابتك اتسجلت ✅ تقدر تغيّرها قبل ما الوقت يخلص (البونص بيتحسب من آخر تغيير)' : 'اختار إجابتك — وتقدر تعدّلها قبل ما الوقت يخلص'}</div>
      </div>`;
    bindHeader(); bindSkip();
    $$('.choice').forEach(btn => btn.onclick = async () => {
      Snd.play('pick');
      $$('.choice').forEach(b => b.classList.toggle('picked', b === btn));
      const r = await act('answer', { choice: parseInt(btn.dataset.c, 10) });
      if (r.ok) {
        S.answeredLocal = true;
        const note = $('#ans-note');
        if (note) note.textContent = 'إجابتك اتسجلت ✅ تقدر تغيّرها قبل ما الوقت يخلص (البونص بيتحسب من آخر تغيير)';
      }
    });
...
function renderConnecting() {
  ...
  const rb = $('#reco-retry'); if (rb) rb.onclick = async () => {
    ...
    LS.del('tahadi_save'); S.save = null; toast(r.gone ? 'الروم القديم خلص' : (r.error || 'مش قادر ترجع'), 'err'); renderHome('');
  };
...
(async function boot() {
  ...
  if (S.save && S.save.code && S.save.token) {
    let r = await api('/api/tahadi/join', { code: S.save.code, token: S.save.token });
    for (let i = 0; i < 3 && r.netErr; i++) { await new Promise(res => setTimeout(res, 700 * (i + 1))); r = await api('/api/tahadi/join', { code: S.save.code, token: S.save.token }); }
    if (r.ok) { openStream(); return; }
...
const ASAL_STEPS = [
  ['🎯', 'اللعبة إيه؟', 'كل واحد فيكم بيجهّز أسئلة معلومات (<span class="hl">اختيار من 3</span>)، وبعدين الأسئلة بتتخلط وتتقلب على الجميع. اللي يجاوب صح ويكون أسرع يكسب أكتر.'],
  ['✍️', 'لما تجهّز أسئلتك', 'اكتب السؤال + <span class="hl">3 اختيارات</span> وعلّم الإجابة الصح، ولازم يكون <span class="hl">من نفس كاتيجوري الخانة</span>. <span class="warn">راجع سؤالك وإجابتك كويس</span> — النظام بيصحّح على اللي علّمته انت، فأي غلطة هتضيّع نقط على صحابك.'],
  ['🎲', 'مش لاقي سؤال؟', 'اسحب واحد جاهز من البنك بإجابته. <span class="hl">ليك 3 تبديلات لكل خانة</span> لو مش عاجبك. وأي سؤال يظهر بيتشال من الروم للأبد — عمره ما هيتكرر.'],
  ['🏃', 'النقط', 'إجابة صح = <span class="hl">100 نقطة + لحد 50 بونص</span>. <span class="hl">البونص بيقل كل ما تتأخر</span>، فالسرعة فرق حقيقي. تقدر تغيّر إجابتك قبل ما الوقت يخلص — بس البونص بيتحسب من آخر تغيير.'],
  ['😏', 'سؤالك مبتجاوبوش', 'لما ييجي سؤال انت اللي كتبته أو سحبته، بتتفرج بس وانت مبسوط — مش هتجاوب على أسئلتك.'],
  ['⚙️', 'الهوست بيحدد', 'الكاتيجوريز، كام سؤال من كل واحدة، <span class="hl">مستوى أسئلة البنك</span> (🟢 سهل / 🟡 متوسط / 🔴 صعب)، ووقت السؤال.'],
  ['👀', 'خلي بالك', 'فوق شريط فيه كل اللاعيبة. اللي يخرج وهو بيجاوب <span class="warn">هتنوّر عليه علامة ❗ قدام الكل</span>. ولو النت قطع ارجع بنفس الجهاز وهتلاقي مكانك زي ما هو، وسكورك محسوب.'],
  ['🏆', 'الآخر', 'بوديوم وجوايز، ومراجعة لكل سؤال بالإجابة الصح ومين جاوب صح ومين غلط.'],
];
function showHelp() {
  const ov = document.createElement('div');
