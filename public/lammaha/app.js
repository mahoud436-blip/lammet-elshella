...
function render() {
  ...
  updPresence(st);
  const key = st.phase + '|' + st.round + '|' + (st.phase === 'clue' ? (st.youAreCluer ? 'c' : 'g') + st.sub + st.cluesGiven + (st.youGuessed ? 'y' : '') : '') + '|' + (st.youReady ? 'r' : '');
  if (key === S.viewKey) {
  ...
  if (st.youAreCluer) {
    if (st.sub === 'hint') {
      app.innerHTML = `
        ...
        ${historyPanel(st)}
        <div class="card">
          <div class="clue-status">✍️ اكتب التلميحة <b>${st.cluesGiven + 1}</b> من ${st.maxClues} <span class="pts-chip">اللي يجيبها منها = ${st.tier}</span></div>
          <div class="center muted small mb">وصّف الكلمة من غير ما تكتبها — النظام هيرفض أي تلميح فيه الاسم أو قريب منه 😏</div>
          <textarea class="field" id="hint-in" maxlength="120" placeholder="مثلًا: لاعب مصري مشهور بيلعب في إنجلترا..."></textarea>
      ...
      app.innerHTML = `
        ...
        ${hintsPanel(st, true)}
        <div class="card">
          <div class="clue-status">👀 بيخمّنوا على تلميحتك رقم ${st.cluesGiven} <span class="pts-chip">اللي يجيبها = ${st.tier} ليك وليه</span></div>
        ...
        <div class="card">
          ...
          ${st.youGuessed
            ? `<div class="center mt" style="font-weight:900;color:var(--brass-hi)">تخمينك: «${esc(st.yourGuessText)}» 🔒</div>
               <div class="center muted small mt">اتقفل — النتيجة هتظهر أول ما الكل يخمّن. تخمينة واحدة لكل تلميحة!</div>`
            : `<div class="center muted small mb">⚠️ ليك تخمينة <b>واحدة بس</b> على التلميحة دي — فكّر كويس!</div>
               <div class="row mt">
    ...
    <div class="card center">
      ...
      ${R.solved
        ? `<div style="font-weight:900;font-size:18px">${R.winners.map(w => `<div>${w.avatar} <span style="color:var(--brass-hi)">${esc(w.name)}</span> جابها! +${R.points}</div>`).join('')}</div>
           <div class="muted mt">و<b>${R.cluerAvatar} ${esc(R.cluerName)}</b> الملمّح خد <b style="color:var(--brass-hi)">+${R.points}</b> (وصّلها بعد ${R.hintsUsed} تلميحة)</div>`
...
const LAMMAHA_STEPS = [
  ['🏠', 'اعملوا روم', 'واحد يعمل روم والباقي (من 3 لـ 12) يدخلوا بالكود أو الـQR — اللعبة أونلاين بالكامل، مش لازم تكونوا مع بعض.'],
  ['⚙️', 'الهوست يظبّط', '<span class="hl">الكاتيجوريز</span> (كورة، بلاد، حيوانات، أكل...)، كام دور يلمّح كل لاعب، <span class="hl">عدد التلميحات (من 1 لـ 10)</span>، مرات تبديل الكلمة الصعبة، والترتيب.'],
  ['🤫', 'كلمة سرية للملمّح', 'كل جولة، لاعب واحد بس بيشوف <span class="hl">اسم سري</span>. وأي اسم اتلعب في الروم <span class="hl">مش بيتكرر تاني أبدًا</span>.'],
  ['✍️', 'يكتب تلميح', 'الملمّح بيكتب تلميح يوصّف بيه الاسم — <span class="warn">والنظام بيرفض أي تلميح فيه الاسم نفسه أو حاجة قريبة منه</span>، فلازم يلمّح من بعيد بذكاء.'],
  ['🔒', 'تخمينة واحدة لكل تلميحة', 'التلميح بيظهر للكل، وكل واحد بيكتب <span class="hl">تخمينة واحدة بس</span> (متشفّرة عن الباقي). النظام هو الحَكَم — بيقبل حتى لو في غلطة إملائية بسيطة أو كتبت بالإنجليزي.'],
  ['🔁', 'كله غلط؟ التلميحة الجاية', 'لو كل التخمينات غلط، بننتقل <span class="hl">تلقائي</span> للتلميحة اللي بعدها والملمّح يكتب واحدة أوضح — لحد ما حد يجيبها أو التلميحات تخلص.'],
  ['🏆', 'سلم النقط', 'من التلميحة الأولى = <span class="hl">100</span>، التانية 90، التالتة 80... العاشرة 10. <span class="hl">كل اللي جابوها صح ياخدوا نقط التلميحة دي، والملمّح ياخد زيهم</span> عشان وصّلهالهم. محدش عرف خالص؟ محدش ياخد حاجة.'],
];
function showHelp() {
