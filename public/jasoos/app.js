...
function render() {
  ...
  updPresence(st);
  const key = st.phase + '|' + st.round + '|' + (st.phase === 'play' ? (st.yourTurn ? 'me' : 'x') + st.turnInRound : '') + '|' + (st.youVoted ? 'v' : '') + (st.youGuessed ? 'g' : '') + (st.youReady ? 'r' : '') + '|' + (st.phase === 'reveal' ? (st.guessOpen ? 'GO' : 'GD') + (st.gameRound || '') + '|c' + (st.creditCount || 0) : '');
  if (key === S.viewKey) {
...
function renderReveal(st) {
  ...
  app.innerHTML = `
    ...
    <div class="card"><h3 class="mb">📊 النقط ${st.isLastRound ? '' : '(تراكمية)'}</h3>${board.map((p, i) => `<div class="rank-row ${p.id === st.you.id ? 'me' : ''}"><span class="pos">${['🥇', '🥈', '🥉'][i] || '#' + (i + 1)}</span><span>${p.avatar}</span><span>${esc(p.name)}${p.left ? ' ' + DOOR_OUT : ''}</span><span class="sc">${p.score}</span></div>`).join('')}</div>
...
function renderGameover(st) {
  ...
  app.innerHTML = `${header('خلصت اللعبة! 🎉')}
    ...
    <div class="card"><h3 class="mb">📊 الترتيب</h3>${R.ranking.map(p => `<div class="rank-row ${p.id === me.id ? 'me' : ''}"><span class="pos">#${p.rank}</span><span>${p.avatar}</span><span>${esc(p.name)}${p.left ? ' <span class="muted small">${DOOR_OUT}</span>' : ''}</span><span class="muted small">(${p.caught} قفشه · ${p.escaped} فلت)</span><span class="sc">${p.score}</span></div>`).join('')}</div>
...
const JASOOS_STEPS = [
  ['🎯', 'اللعبة إيه؟', 'كله شايف <span class="hl">كلمة سرية</span> ما عدا <span class="warn">الجاسوس</span> — هو شايف الكاتيجوري بس. بالدور كل واحد بيكتب <span class="hl">كلمة واحدة</span> توصف الكلمة، والجاسوس بيمثّل إنه عارف. المحققين بيكسبوا لو عرفوا الجاسوس، والجاسوس بيكسب لو فلت.'],
  ['✍️', 'دورك: كلمة واحدة', 'تكتب <span class="hl">كلمة واحدة بس</span> توصف الكلمة السرية. <span class="warn">ممنوع تكتب الكلمة نفسها أو حاجة قريبة منها</span>، وممنوع تكرر كلمة اتقالت قبلك.'],
  ['⏱️', 'الكلمة بتختفي!', 'الكلمة السرية بتظهرلك <span class="hl">10 ثواني بس</span> في الأول وبعدين تختفي — ركّز واحفظها، مفيش سجل ترجعله.'],
  ['🤔', 'دي اللعبة الحقيقية', 'لو وصفت الكلمة بدقة زيادة، الجاسوس هيعرفها. ولو غمّضت أوي، صحابك هيشكّوا فيك انت. <span class="hl">لازم توازن</span> بين الاتنين.'],
  ['🗳️', 'التصويت', 'بعد ما الكل يخلّص كلماته، كل محقق بيصوّت <span class="hl">بعدد الجواسيس</span> (واحد أو أكتر). بتاخد <span class="hl">100 نقطة عن كل جاسوس تصيبه صح</span>. الجاسوس مبيصوّتش — بيمثّل إنه بيصوّت 😏'],
  ['🎭', 'النتيجة الأول', 'بعد التصويت بتظهر النتيجة: مين كان الجاسوس ومين قفشه. نقط الجاسوس: <span class="hl">فلت من الكل = 100</span>، فلت من بعضهم = 50، قفشوه كلهم = صفر.'],
  ['💯', 'وبعدين تخمين الجاسوس', 'بعد ما النتيجة تتكشف، الجاسوس بياخد فرصة يخمّن الكلمة السرية — لو عرفها <span class="hl">+100 نقطة كمان</span> (مش قبل النتيجة).'],
  ['🔁', 'كذا جولة والنقط تتجمع', 'اللعبة <span class="hl">كذا جولة</span> — كل جولة جاسوس عشوائي جديد وكلمة جديدة، والنقط بتتراكم لحد آخر الجيم، وبعدين تحليل النتايج والجوايز.'],
  ['🕵️', 'عدد الجواسيس', '3 لاعيبة → جاسوس واحد. من 4 لـ 6 → لحد 2. من 7 وفوق → لحد 3. ولو الهوست اختار <span class="hl">عشوائي</span>، محدش يعرف كانوا كام غير في النهاية 🤫'],
  ['⚙️', 'الهوست بيحدد', 'الكاتيجوريز، <span class="hl">كام كلمة يكتب كل لاعب (2–6)</span>، <span class="hl">عدد الجولات في الجيم (1–10)</span>، عدد الجواسيس (يحدده ولا عشوائي سري)، ووقت الدور.'],
  [DOOR_OUT, 'حاجات مهمة', 'الكلمات متتكررش في الروم أبدًا. ولو خرجت سكورك بيفضل محسوب. وفوق شريط بيوضّح لو حد خرج من اللعبة وهو بيلعب ❗'],
];
function showHelp() {
