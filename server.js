...
const GAMES = {
  ...
  jasoos: require('./server/jasoos/engine'),
  conan: require('./server/conan/engine'),
};
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const m = pathname.match(/^\/api\/(tahadi|wisper|lammaha|jasoos|conan)\/(create|join|action|stream)$/);
    if (m) {
    ...
    if (req.method === 'GET' || req.method === 'HEAD') {
      ...
      if (fp === '/jasoos') return (res.writeHead(302, { Location: '/jasoos/' }), res.end());
      if (fp === '/conan') return (res.writeHead(302, { Location: '/conan/' }), res.end());
      if (fp === '/tahadi/') fp = '/tahadi/index.html';
      if (fp === '/wisper/') fp = '/wisper/index.html';
      if (fp === '/lammaha/') fp = '/lammaha/index.html';
      if (fp === '/jasoos/') fp = '/jasoos/index.html';
      if (fp === '/conan/') fp = '/conan/index.html';
      ...
      fs.access(full, fs.constants.R_OK, (err) => {
        ...
        else if (pathname.startsWith('/jasoos/')) fb = 'jasoos/index.html';
        else if (pathname.startsWith('/conan/')) fb = 'conan/index.html';
...
function listen(port, attempt) {
  ...
  server.listen(port, '0.0.0.0', () => {
    ...
    pushNet(port);
    console.log('');
    console.log('  🎪 لمّة الشلة شغّالة! (5 ألعاب)');
    console.log('  ──────────────────────────────');
    if (HOSTED) {
